    ...
    public InteractionResult onItemUseFirst(ItemStack stack, UseOnContext context) {
        ...
        if (!level.isClientSide && player != null) {
            ...
            if (blockEntity instanceof RouterBlockEntity routerBE) {
                if (!heldItem.hasTag()) {
                    sendGameInfoMessage(player, "message.cdm.invalid_cable");
                    ...
                    if (!router.isDeviceRegistered(networkDeviceBlockEntity)) {
                        if (router.addDevice(networkDeviceBlockEntity)) {
                            ...
                            if (getDistance(tileEntity1.getBlockPos(), routerBE.getBlockPos()) > DeviceConfig.SIGNAL_RANGE.get()) {
                                sendGameInfoMessage(player, "message.cdm.successful_registered");
                            } else {
                                sendGameInfoMessage(player, "message.cdm.successful_connection");
                            }
                        } else {
                            sendGameInfoMessage(player, "message.cdm.router_max_devices");
                        }
                    } else {
                        sendGameInfoMessage(player, "message.cdm.device_already_connected");
                    }
                } else {
            ...
            if (blockEntity instanceof NetworkDeviceBlockEntity networkDeviceBlockEntity) {
                ...
                sendGameInfoMessage(player, "message.cdm.select_router");
                return InteractionResult.SUCCESS;
            }
        }
        return InteractionResult.SUCCESS;
